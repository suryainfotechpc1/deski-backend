...
class AdminController extends Controller
{
	...
	public function addMenu(Request $request){
		...
		if($id){
			...
			// $menu_obj = Menu::where('name',$menu)->first();
			...
			// }else{
				// return redirect()->back()->with('update', 'Menu already exists.');
			// }
		...
		for($i = 0; $i < count($request->box_title); $i++){
			...
			if ($request->hasFile('img')) {
				...
				foreach($files as $file){
				    ...
				    $fileName = time().'_sideImage.'.$extension;
				    $file->move(public_path('images/sideImamge'), $fileName);
				    $destinationPath = asset('images/sideImamge').'/'.$fileName;
				}
			}
	...
	public function saveAboutFounder(Request $request) {
		$img_arr = [];
		$files = $request->file('side_img');
		
		$json_file = [];
		$about = PageContent::where('page', 'about')->where('section_id', 3)->first();
		if($about){
			$json_file = json_decode($about->side_img);
		}
		...
		for($i=0; $i<4; $i++){
			if(isset($files[$i])){
				...
				$fileName = time().'_member.'.$extension;
				$files[$i]->move(public_path('images/team'), $fileName);
				$img_arr[] = asset('images/team').'/'.$fileName;
			}else {
				$img_arr[] = @$json_file[$i] ?? '';
			}
		}
	...
	public function addCategory(Request $request){
		...
		if($id){
			...
			$menu_obj->slug = ".".str_replace(' ', '_', strtolower($menu));
			$menu_obj->type = $type;
			$menu_obj->save();
			return redirect()->back();
		}else{
			...
			$addmenu->slug = ".".str_replace(' ', '_', strtolower($menu));
			$addmenu->type = $type;
			$addmenu->save();
			// return redirect()->back()->with('success', 'Menu added Successfully.');
	...
	public function getPortfolio(){
		...
		$portfolionew = Portfolio::where('is_deleted','0')->where('type', 'portfolios')->get();
		$catde = CategoryPort::where('is_deleted','0')->where('type', 'portfolios')->get();
	...
	public function add_portfolio(Request $request){
		$id = $request->id;
		$servicesedit = Portfolio::where('is_deleted','0')->where('id', $id)->where('type', 'portfolios')->get();
		$catde = CategoryPort::where('is_deleted','0')->where('type', 'portfolios')->get();
		return view('admin.add_portfolio',['servicesedit' => $servicesedit, 'catde' => $catde]);
	}
	
	public function getViewPortfolio(){
		$portfolionew = Portfolio::where('is_deleted','0')->where('type', 'portfolios')->get();
		return view('admin.view_portfolio',['portfolionew' => $portfolionew]);
	}
	
	public function getBlog(){
		$portfolio = PageContent::where('page', 'blog')->get();
		$portfolionew = Portfolio::where('is_deleted','0')->where('type', 'blog')->get();
		$catde = CategoryPort::where('is_deleted','0')->where('type', 'blog')->get();
		
		return view('admin.blog',['portfolio' => $portfolio, 'portfolionew' => $portfolionew, 'catde' => $catde]);
	}
	
	public function add_blog(Request $request){
		$id = $request->id;
		$servicesedit = Portfolio::where('is_deleted','0')->where('id', $id)->where('type', 'blog')->get();
		$catde = CategoryPort::where('is_deleted','0')->where('type', 'blog')->get();
		return view('admin.add_blog',['servicesedit' => $servicesedit, 'catde' => $catde]);
	}
	
	public function getViewBlog(){
		$portfolionew = Portfolio::where('is_deleted','0')->where('type', 'blog')->get();
		return view('admin.view_blog',['portfolionew' => $portfolionew]);
	}
	...
	public function addPortfolio(Request $request){
		...
		if($id){
				...
				$menu_obj->category_id = $cat;
				$menu_obj->title = $title;
				$menu_obj->content = $request->section_content;
			...
			$pageSection->title = $request->title;
			$pageSection->category_id = $request->category_id;
